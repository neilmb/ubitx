/**
The CAT protocol is used by many radios to provide remote control to computers
...
byte getHighNibble(byte b) { return (b >> 4) & 0x0f; }

byte getLowNibble(byte b) { return b & 0x0f; }
...
unsigned long readFreq(byte* cmd) {
  ...
  return (unsigned long)d7 * 100000000L + (unsigned long)d6 * 10000000L +
         (unsigned long)d5 * 1000000L + (unsigned long)d4 * 100000L +
         (unsigned long)d3 * 10000L + (unsigned long)d2 * 1000L +
         (unsigned long)d1 * 100L + (unsigned long)d0 * 10L;
}
...
void checkCAT() {
  ...
  cat[3] = cat[2];
  cat[2] = cat[0];
  for (i = 0; i < 5; i++) cat[i] = Serial.read();
