...
extern char printBuff[2][17],  b[30], c[30], inTx, ritOn, isUSB, vfoActive;
extern unsigned long cwTimeout, frequency;
...
void printLine(char linenmbr, char *c) {
  if (strcmp(c, printBuff[(int) linenmbr])) {     // only refresh the display when there was a change
    lcd.setCursor(0, linenmbr);             // place the cursor at the beginning of the selected line
    lcd.print(c);
    strcpy(printBuff[(int)linenmbr], c);
...
int enc_read(void) {
  ...
  unsigned long stop_by = millis() + 50UL;
