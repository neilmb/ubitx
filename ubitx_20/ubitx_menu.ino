...
- The main loop() constantly looks for a button press and calls doMenu() when it
detects a function button press.
- As the encoder is rotated, at every 10th pulse, the next or the previous menu
item is displayed. Each menu item is controlled by its own function.
...
void menuBand(int btn) {
  ...
  while (!btnDown()) {
    ...
    if (knob != 0) {
      ...
      if (knob < 0 && frequency > 3000000L) setFrequency(frequency - 200000L);
      if (knob > 0 && frequency < 30000000L) setFrequency(frequency + 200000L);
...
So, if the vco is shifted up by 875 Hz, the generated frequency of 1 MHz is
shifted by 1 Hz (875/875) At 12 MHz, the carrier will needed to be shifted down
by 12 Hz for every 875 Hz of shift up of the vco
*/
...
void doMenu() {
  ...
  while (menuOn) {
    ...
    if (i > 0) {
      if (modeCalibrate && select + i < 110) select += i;
      if (!modeCalibrate && select + i < 70) select += i;
    }
    if (i < 0 && select - i >= 0)
