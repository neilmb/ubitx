...
When a straight key is connected, it shorts the pull-up resistor, analog input
is 0 volts When a paddle is connected, the dot and the dash are connected to the
analog pin through a 10K and a 2.2K resistors. These produce a 4v and a 2v input
to the analog pins. So, the readings are as follows :
 - 0v - straight key
 - 1-2.5 v - paddle dot
...
// in milliseconds, this is the parameter that determines how long the tx will
// hold between cw key downs
...
Each time the key is hit the cwTimeout is pushed to a time in the future by
cwKeyDown()
*/
void cwKeyer() {
  byte paddle;
  ...
  while (1) {
    ...
    if (!paddle) {
      ...
      // if a paddle was used (not a straight key) we should extend the space to
      // be a full dash  by adding two more dots long space (one has already
      // been added at the end of the dot or dash)
      if (cwTimeout > 0 && lastPaddle != PADDLE_STRAIGHT) delay(cwSpeed * 2);

      // got back to the begining of the loop, if no further activity happens on
      // the paddle or the straight key we will time out, and return out of this
      // routine
      delay(5);
      continue;
    }
